...
\begin{document}
%\begin{enumerate} 
%\item The propagation delays of the XOR gate,AND gate and multiplexer(MUX)in the circuit shown in the figure are4ns,2ns and 1ns,respectively.
\begin{tikzpicture}
\ctikzset{
logic ports=ieee,
...
\draw (a.in 2) -| (b.in 1);
\draw (b.in 2) -| (c.in 1);
\draw (b.out) -- ++(1.3,0) node{0};
\draw (c.out) -- ++(1.3,0) node{1};
\draw (e.out) -- ++(0.5,0) node{1};
\draw (a.out) -- ++(6.5,0) node{0};
